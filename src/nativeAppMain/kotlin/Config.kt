...
object ConfigLoader {
    ...
    private fun loadFromEnv(): AppConfig {
        val sheetId = getEnv("GOOGLE_SHEET_ID")
            ?: "1emW2Zsb0gEtEHiub_YHpazvBd4lL4saxCwyPhbtxXYM"
        ...
        return AppConfig(
            sheet = SheetConfig(
                id = sheetId,
                index = getEnv("GOOGLE_SHEET_INDEX")?.toIntOrNull() ?: 0,
                headerRow = getEnv("HEADER_ROW")?.toIntOrNull() ?: 2,
                titleColumn = getEnv("TITLE_COLUMN")?.toIntOrNull() ?: 0
            ),
            git = GitConfig(
                repoUrl = repoUrl,
                token = gitToken,
